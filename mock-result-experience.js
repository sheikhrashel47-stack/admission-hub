...
(() => {
  ...
  function renderDashboard(result) {
    ...
    const primary = matched.slice().sort((a, b) => Number(a?.priority || 9) - Number(b?.priority || 9))[0] || null;
    ...
    const teacherInsight = primary ? `<div class="result-analysis-insight-head"><span>${safe(primary.family || 'Matched analysis')}</span><b>${safe(primary.id)}</b></div>${primary.title ? `<h4 class="result-analysis-title">${safe(primary.title)}</h4>` : ''}<p>${safe(primary.analysis || '')} ${safe(primary.reason || '')}</p><div class="result-analysis-action"><strong>পরবর্তী পদক্ষেপ</strong><span>${safe(primary.action || '')}</span></div><div class="result-analysis-motivation"><strong>অনুপ্রেরণা</strong><span>${safe(primary.motivation || '')}</span></div>` : `<div class="result-analysis-insight-head"><span>Teacher insight</span><b>LIVE</b></div><p>${safe(insight)}</p>`;
