...
(() => {
  ...
  function matchedAnalyses(result) {
    ...
    const byId = new Map(analysisRows().map(row => [row.id, row]));
  ...
  function renderDashboard(result) {
    ...
    const matched = matchedAnalyses(result);
    ...
    const insight = primary ? `${primary.analysis} ${primary.reason}` : template.replace(/\{(overall|participation|comparison|subjectSignal|topicSignal|paceSignal)\}/g, (_, key) => variables[key]);
    const analysisCards = matched.map((item, index) => `<article class="result-analysis-insight ${index === 0 ? 'is-primary' : ''}"><div class="result-analysis-insight-head"><span>${safe(item.family || 'Matched analysis')}</span><b>${safe(item.id)}</b></div><p>${safe(item.analysis)} ${safe(item.reason)}</p><div class="result-analysis-action"><strong>পরবর্তী পদক্ষেপ</strong><span>${safe(item.action)}</span></div><div class="result-analysis-motivation"><strong>অনুপ্রেরণা</strong><span>${safe(item.motivation)}</span></div></article>`).join('');
