...
(function () {
  ...
  window.ahDeleteSubject = async id => { const s = CACHE.subjects.find(x => x.id === id); if (!s) return; const topicIds = new Set((CACHE.topics || []).filter(t => t.subjectId === id).map(t => t.id)); const questions = (CACHE.questions || []).filter(q => q.subjectId === id || topicIds.has(q.topicId)); if (!confirm(`Delete ${s.name} permanently? ${topicIds.size} topics and ${questions.length} questions will be permanently deleted. This cannot be undone.`)) return; try { for (const q of questions) await dbDel('questions', q.id); for (const topicId of topicIds) await dbDel('topics', topicId); await dbDel('subjects', id); await refresh(); toast(`${s.name} and its content were permanently deleted.`); } catch (error) { console.error('ahDeleteSubject failed', error); toast('Subject could not be deleted. No records were moved.'); } };
  ...
  window.ahDeleteTopic = async id => { const t = CACHE.topics.find(x => x.id === id); if (!t) return; const childIds = typeof window.topicHierarchy?.descendantIds === 'function' ? window.topicHierarchy.descendantIds(id) : [id]; const ids = new Set(childIds); const questions = (CACHE.questions || []).filter(q => ids.has(q.topicId) || (q.subjectId === t.subjectId && q.topicId === id)); const count = questions.length; const topicCount = ids.size; if (!confirm(`Delete ${t.name} permanently? ${topicCount} topic${topicCount === 1 ? '' : 's'} and ${count} question${count === 1 ? '' : 's'} will be permanently deleted. This cannot be undone.`)) return; try { for (const q of questions) await dbDel('questions', q.id); for (const topicId of [...ids].reverse()) await dbDel('topics', topicId); await refresh(); toast(`${topicCount > 1 ? 'Topic tree' : 'Topic'} and its questions were permanently deleted.`); } catch (error) { console.error('ahDeleteTopic failed', error); toast('Topic could not be deleted. No questions were moved.'); } };
