...
(function () {
  ...
  const state = { format: 'text', text: '', fileName: '', rows: [], rejected: [], parserMode: false, subjectId: '', topicId: '', successMessage: '', successAdded: 0, successSkipped: 0 };
  ...
  function parserTopics(subjectId) { return (CACHE.topics || []).filter(t => t.subjectId === subjectId).sort((a, b) => (a.order || 0) - (b.order || 0)); }
  ...
  window.ahSetSubject = value => { state.subjectId = value; state.topicId = ''; render(); };
  window.ahSetTopic = value => { state.topicId = value; render(); };
  window.ahAddSubject = async () => { const name = prompt('New subject name', '')?.trim(); if (!name) return; const existing = CACHE.subjects.find(s => s.name.trim().toLowerCase() === name.toLowerCase()); if (existing) { state.subjectId = existing.id; state.topicId = ''; render(); return; } const subject = { id: uidA(), name, icon: '📘', color: '#0f6b4f', order: CACHE.subjects.length, createdAt: Date.now() }; await put('subjects', subject); await loadCache(); state.subjectId = subject.id; state.topicId = ''; render(); };
  window.ahAddTopic = async () => { if (!state.subjectId) return toast?.('Select a Subject first'); const name = prompt('New topic name', '')?.trim(); if (!name) return; const existing = CACHE.topics.find(t => t.subjectId === state.subjectId && t.name.trim().toLowerCase() === name.toLowerCase()); if (existing) { state.topicId = existing.id; render(); return; } const topic = { id: uidA(), subjectId: state.subjectId, name, order: parserTopics(state.subjectId).length, createdAt: Date.now(), updatedAt: Date.now() }; await put('topics', topic); await loadCache(); state.topicId = topic.id; render(); };
  function parserView() {
  ...
  window.ahResetParser = () => { state.rows = []; state.rejected = []; state.text = ''; state.subjectId = ''; state.topicId = ''; state.successMessage = ''; render(); };
  window.ahConfirmImport = async () => { const subject = CACHE.subjects.find(s => s.id === state.subjectId); const topic = CACHE.topics.find(t => t.id === state.topicId && t.subjectId === state.subjectId); if (!subject || !topic) return toast?.('Select a Subject and Topic before importing'); const rows = state.rows.filter(x => x.answer >= 0 && x.options.filter(Boolean).length === 4); if (!rows.length) return toast?.('No valid questions to import'); const existing = new Set((CACHE.questions || []).map(q => parserQuestionKey(q.question))); const seen = new Set(); let added = 0, skipped = 0; let nextNumber = nextQuestionNumberForTopic(topic.id); for (const row of rows) { const key = parserQuestionKey(row.question); if (!key || existing.has(key) || seen.has(key)) { skipped++; continue; } await put('questions', { id: uidA(), subjectId: subject.id, topicId: topic.id, questionNumber: nextNumber++, question: row.question, options: row.options, answer: row.answer, explanation: row.explanation, stats: { attempts: 0, correct: 0, wrong: 0 }, bookmarked: false, createdAt: Date.now(), updatedAt: Date.now(), source: 'question-parser' }); existing.add(key); seen.add(key); added++; } await loadCache(); state.successAdded = added; state.successSkipped = skipped; state.successMessage = `${added} questions successfully added to ${subject.name} → ${topic.name}`; state.rows = []; state.rejected = []; state.text = ''; render(); };
  ...
  function hook() { const old = window.render; if (!old || window.__ahFeatureRender) return; window.__ahFeatureRender = true; window.render = function () { const p = Router.path; if (p === 'question-parser' || p === 'smart-formatter') return renderShell(parserView(), { title: 'Question Parser', back: "navigate('dashboard')" }); if (p.startsWith('question-bank/subject/') || p.startsWith('question-bank/topic/')) { if (window.renderQuestionBankV2) return window.renderQuestionBankV2(); } if (p === 'question-bank' || p.startsWith('question-bank/')) { if (window.renderQuestionBankV2) return window.renderQuestionBankV2(); return renderShell(bankView(), { title: 'Question Bank' }); } if (p === 'rewards' || p === 'reward-shop') return rewardsView(); return old.apply(this, arguments); }; patchExam(); }
