...
(() => {
  ...
  const appCache = () => { try { return typeof CACHE !== 'undefined' ? CACHE : window.CACHE; } catch (_) { return window.CACHE; } };
  ...
  style.textContent = `
    ...
    [data-theme="midnight-academic"] .card,[data-theme="midnight-academic"] .topbar,[data-theme="midnight-academic"] .bottomnav,[data-theme="midnight-academic"] .command-tile,[data-theme="midnight-academic"] .command-card,[data-theme="midnight-academic"] .metric-card,[data-theme="midnight-academic"] .premium-card,[data-theme="midnight-academic"] .premium-hero,[data-theme="midnight-academic"] .listitem,[data-theme="midnight-academic"] .p3-command-section-v3,[data-theme="midnight-academic"] .p3-command-card-v3,[data-theme="midnight-academic"] .q-bank-container,[data-theme="midnight-academic"] .q-card-v2{background:var(--card)!important;color:var(--text)!important;border-color:var(--line)!important}
    [data-theme="midnight-academic"] .muted,[data-theme="midnight-academic"] .p3-muted,[data-theme="midnight-academic"] small{color:var(--sub)!important}
    [data-theme="midnight-academic"] .command-tile .label,[data-theme="midnight-academic"] .command-card .label,    [data-theme="midnight-academic"] .q-text-v2,[data-theme="midnight-academic"] .q-card-meta,[data-theme="midnight-academic"] .q-card-footer{color:var(--text)!important}
    [data-theme="midnight-academic"] .result-filter,[data-theme="midnight-academic"] .result-option,[data-theme="midnight-academic"] .q-opt-v2,[data-theme="midnight-academic"] .flash-opt{background:#102b52!important;color:var(--text)!important;border-color:var(--line)!important}
    [data-theme="midnight-academic"] .result-option.is-correct,[data-theme="midnight-academic"] .flash-opt.correct,[data-theme="midnight-academic"] .q-opt-v2.correct{background:#123e39!important;color:#dfffee!important;border-color:#48d69a!important}
    [data-theme="midnight-academic"] .result-option.is-wrong,[data-theme="midnight-academic"] .flash-opt.wrong,[data-theme="midnight-academic"] .q-opt-v2.wrong{background:#4a2630!important;color:#ffe7ea!important;border-color:#ff7777!important}
    [data-theme="midnight-academic"] .result-explanation,[data-theme="midnight-academic"] .flash-feedback,[data-theme="midnight-academic"] .q-explanation-v2{background:#263a55!important;color:#f1f6ff!important;border-color:var(--line)!important}
    [data-theme="midnight-academic"] .result-option-note,[data-theme="midnight-academic"] .result-answer-line{color:var(--text)!important}
  ...
  const liveStats = () => {
    const c = appCache() || {};
    const results = Array.isArray(c.examResults) ? c.examResults : [];
    ...
    const xp = Number(c.settings?.totalXpEarned || c.settings?.xpBalance || localStorage.getItem('xp') || 0) || 0;
    ...
    const rawName = String(profile?.name || c.settings?.userName || 'Zayan');
  ...
  const hydrateQuestionBank = async () => {
    ...
    const c = appCache();
    if (!c) return;
    const needsRender = /No subjects found|প্রশ্ন ব্যাংক/.test(document.body?.innerText || '') && !document.querySelector('.ah-subject-grid,.q-bank-container .q-feed-body');
    if (Array.isArray(c.questions) && c.questions.length) {
      if (needsRender && typeof window.__admissionRenderRoute === 'function') window.__admissionRenderRoute();
      return;
    }
    try {
      /* loadCache updates the app's real top-level CACHE binding; this is important because
         CACHE may be declared as a lexical global rather than a window property. */
      if (typeof window.loadCache === 'function') await window.loadCache();
      const hasQuestions = Array.isArray(c.questions) && c.questions.length;
      if (hasQuestions) {
        if (typeof window.__admissionRenderRoute === 'function') window.__admissionRenderRoute(); else if (typeof window.render === 'function') window.render();
      }
    } catch (e) { console.warn('[Juju] Question Bank hydration failed', e); }
  };
  const ensureQuestionBankTopic = () => {
    const p = path();
    if (!p.startsWith('question-bank/topic/')) return;
    const c = appCache();
    const tid = decodeURIComponent(p.split('/')[2] || '');
    const topic = c?.topics?.find(t => t.id === tid);
    const subject = c?.subjects?.find(s => s.id === topic?.subjectId);
    const questions = Array.isArray(c?.questions) ? c.questions.filter(q => q.topicId === tid).slice(0, 30) : [];
    if (!topic || !questions.length || document.querySelector('.q-feed-body .juju-bank-card')) return;
    const cards = questions.map((q,i) => `<article class="card juju-bank-card"><div class="row between"><b>প্রশ্ন ${i+1}</b><span class="muted">${escV(subject?.name||'')} · ${escV(topic.name||'')}</span></div><h3>${escV(q.question)}</h3><div class="juju-bank-options">${(q.options||[]).map((o,j)=>`<button type="button" class="q-opt-v2" onclick="selectTopicAnswer?.('${escV(q.id)}',${j})"><b>${String.fromCharCode(65+j)}</b><span>${escV(o)}</span></button>`).join('')}</div>${q.explanation?`<p class="muted">${escV(q.explanation)}</p>`:''}</article>`).join('');
    if (typeof renderShell === 'function') renderShell(`<div class="q-bank-container"><header class="q-bank-header"><div class="row"><button class="q-back-btn" type="button" onclick="history.back()">‹</button><div><h1>${escV(topic.name)}</h1><p>Practice / ${escV(subject?.name||'')} · ${questions.length}+ questions</p></div></div></header><div class="q-feed-summary"><span>${questions.length} questions shown</span><span>Live Question Bank</span></div><div class="q-feed-body">${cards}</div></div>`,{title:topic.name,back:'history.back()'});
  };
  const ensureQuestionBankRoot = () => {
    if (path() !== 'question-bank') return;
    const c = appCache();
    if (!c || !Array.isArray(c.subjects) || !c.subjects.length || document.querySelector('[data-qnav-card]')) return;
    const subjects = [...c.subjects].sort((a,b)=>(a.order||0)-(b.order||0));
    const cards = subjects.map(s => {
      const count = Array.isArray(c.questions) ? c.questions.filter(q => q.subjectId === s.id).length : 0;
      const topics = Array.isArray(c.topics) ? c.topics.filter(t => t.subjectId === s.id).length : 0;
      return `<button type="button" class="q-nav-card card" data-qnav-card onclick="openRedesignedSubject('${escV(s.id)}')"><span class="q-nav-badge" style="background:#167be1">${escV(s.icon||'📘')}</span><span class="q-nav-info"><strong>${escV(s.name)}</strong><span>${topics} Topics · ${count} Questions</span></span><span class="q-nav-arrow">›</span></button>`;
    }).join('');
    if (typeof renderShell === 'function') renderShell(`<div class="q-bank-container"><header class="q-bank-header"><div class="row between"><div><h1>প্রশ্ন ব্যাংক</h1><p>সব Subject · ${Array.isArray(c.questions)?c.questions.length:0}টি প্রশ্ন</p></div><div class="row"><button class="q-header-icon" type="button" onclick="toggleQSearch?.()">🔍</button><button class="q-header-icon" type="button" onclick="openQuestionBankSettings?.()">⚙️</button></div></div></header><div class="q-list-body ah-subject-grid">${cards}</div></div>`,{title:'Question Bank',back:"navigate('dashboard')"});
  };
  const cleanResult = () => {
