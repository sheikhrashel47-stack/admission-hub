(function(){
  ...
  function ownedRewards(){
    const inv=settings().rewardInventory||{};
    return Object.entries(inv).filter(([,v])=>v&&v.ownedAt).map(([id,v])=>({id,name:rewardNames[id]||id.replace(/[-_]/g,' '),active:!!v.active,remaining:v.remaining??'∞',expiry:v.expiresAt?new Date(v.expiresAt).toLocaleDateString():'No expiry'}));
  }
