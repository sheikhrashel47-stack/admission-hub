...
(function installNotesTool(){
  ...
  function addDashboardCard(){
    ...
    const section=[...document.querySelectorAll('.dashboard-section')].find(el=>el.textContent.includes('Study Tools') && el.textContent.includes('Review & analysis'));
    if(!section || section.querySelector('[data-notes-tool-card]')) return;
    ...
    const grid=section.querySelector('.special-tools-grid'); if(grid) grid.appendChild(card); else section.appendChild(card);
  }
