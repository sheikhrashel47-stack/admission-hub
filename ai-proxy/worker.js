...
function buildTask(payload) {
  ...
  const subjects = Array.isArray(payload?.subjectPerformance) ? payload.subjectPerformance : [];
  const topics = Array.isArray(payload?.topicPerformance) ? payload.topicPerformance : [];
  const compact = { result, previousResults: previous, subjectPerformance: subjects, topicPerformance: topics };
  return `তুমি Admission Hub-এর একজন দক্ষ, সহানুভূতিশীল exam mentor। নিচের VERIFIED JSON result summary বিশ্লেষণ করো। কোনো website খুলবে না, কোনো বাইরের তথ্য ব্যবহার করবে না, এবং কোনো সংখ্যা বানাবে না। App-এর calculated score, accuracy, negative marking, current result ও previous result পরিবর্তন করবে না।
...
{"headline":"...","summary":"...","strengths":["..."],"weaknesses":["..."],"mistakePattern":"...","comparison":"...","targetAdvice":"...","plan1Day":"...","plan3Days":"...","plan7Days":"...","motivation":"...","visualInsights":[{"label":"...","value":"...","tone":"strong|watch|focus"}]}

Verified JSON:
${JSON.stringify(compact)}`.slice(0, MAX_TASK_CHARS);
}
