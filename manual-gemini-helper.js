...
(function installManualGeminiHelper(){
  ...
  function buildPrompt({q, selectedIndex, source}){
    ...
    return `ভুল MCQ বিশ্লেষণ করো। শুধু প্রশ্ন, অপশন ও উৎস ব্যবহার করো; সঠিক উত্তর বদলাবে না, কিছু বানাবে না। Greeting/ভূমিকা/repetition বাদ।

Output: ৫–১০টি ছোট লাইন, ৮০–১২০ শব্দ। এই ৬টি label সর্বোচ্চ একবার ব্যবহার করো: কেন ভুল, সঠিক নিয়ম, Exam Trap, মনে রাখার কৌশল, ১টি উদাহরণ, Final takeaway। প্রতি label-এ ১–২টি বাক্য। ভুলের কারণ, concept ও একই ভুল এড়ানোর উপায় বলো। ambiguity/conflict থাকলে এক লাইনে বলো।
...
শুধু বিশ্লেষণ দাও; অতিরিক্ত তথ্য নয়।`;
  }
  ...
  const GEMINI_PAGE_STATE='admission_gemini_mistake_v1';
  function savePageState(){
    try { sessionStorage.setItem(GEMINI_PAGE_STATE, JSON.stringify({prompt:window.__manualGeminiLastPrompt||'',context:window.__manualGeminiLastContext||{},source:window.__manualGeminiSource||'',returnPath:window.__manualGeminiReturnPath||'dashboard'})); } catch (_) {}
  }
  ...
  function showPrompt({q, selectedIndex, source}){
    ...
    setTimeout(() => addInlineNoteButton({q}), 80);
    ...
    root.querySelectorAll('.mg-copy,.mg-gemini,.mg-note').forEach(button => Object.assign(button.style,{flex:'1 1 180px',minHeight:'42px',border:'0',borderRadius:'12px',padding:'11px',fontWeight:'900',fontSize:'12px',lineHeight:'1.3',whiteSpace:'normal'}));
    ...
    const noteButton = root.querySelector('.mg-note'); if (noteButton) Object.assign(noteButton.style,{background:'#edf7f1',color:'#0f6b4f'});
  ...
  window.openManualGemini = function(){
    ...
    if (typeof window.navigate === 'function') window.navigate('gemini');
    else location.hash = '#gemini';
  };
  window.openGeminiWebsite = function(){
    savePageState();
    const go = () => window.location.assign('https://gemini.google.com/app');
