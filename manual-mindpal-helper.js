...
(function installManualMindPalHelper(){
  ...
  function showPrompt({q, selectedIndex, source}){
    ...
    root.querySelectorAll('.mmp-copy,.mmp-mindpal,.mmp-close-btn').forEach(button=>Object.assign(button.style,{flex:'1 1 180px',minHeight:'42px',border:'0',borderRadius:'12px',padding:'11px',fontWeight:'900',fontSize:'12px',lineHeight:'1.3',whiteSpace:'normal'}));
  ...
  window.openManualMindPal = function(){
    ...
    const mindPalUrl='https://chatbot.getmindpal.com/university-admission-mistake-analysis-agent-fog';
    const opened=window.open(mindPalUrl,'_blank');
    if(opened){ try { opened.opener=null; } catch (_) {} }
    else window.location.assign(mindPalUrl);
  };
