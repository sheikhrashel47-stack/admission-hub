(() => {
  ...
  function rewardCatalogU() {
    ...
    const seen = new Set();
    return merged.filter(r => { if (!r?.id || seen.has(r.id)) return false; seen.add(r.id); return true; }).map(r => { const currency = ['Gold','Diamond','XP'].includes(r.currency) ? r.currency : 'Gold'; return { ...r, currency, price: Math.max(1, Math.round(numU(r.price) || 100)) }; });
  }
  window.__upgradeExtraRewards = extraRewards;
  let selectedUpgradeCategoryU = '';
  ...
  async function buyRewardU(id) {
    const reward = rewardCatalogU().find(r => r.id === id); const g = gameU(); const s = settingsU();
    if (!reward) return;
    if (rewardOwnedU(g, id)) { toastU('এই reward আগে থেকেই তোমার আছে।'); return; }
    const currency = ['Gold','Diamond','XP'].includes(reward.currency) ? reward.currency : 'Gold';
    const balance = currency === 'XP' ? numU(s.xpBalance) : currency === 'Diamond' ? numU(g.diamonds) : numU(g.gold);
    if (balance < reward.price) { toastU(`আরও ${reward.price - balance} ${currency} প্রয়োজন।`); return; }
    if (currency === 'XP') { s.xpBalance = balance - reward.price; s.totalXpSpent = numU(s.totalXpSpent) + reward.price; }
    else if (currency === 'Diamond') { g.diamonds = balance - reward.price; s.totalDiamondSpent = numU(s.totalDiamondSpent) + reward.price; }
    else { g.gold = balance - reward.price; s.totalGoldSpent = numU(s.totalGoldSpent) + reward.price; }
    g.inventory[id] = { ownedAt: nowU(), used: 0, remaining: null };
    g.transactions.unshift({ id: `currency-buy-${nowU()}`, date: nowU(), xp: currency === 'XP' ? -reward.price : 0, gold: currency === 'Gold' ? -reward.price : 0, diamonds: currency === 'Diamond' ? -reward.price : 0, label: `-${reward.price} ${currency} — ${reward.name}`, source: `upgrade-purchase-${id}` });
    await saveSettingsU();
    toastU(`${reward.name} ${currency} দিয়ে কেনা হয়েছে।`);
    renderUpgradeRouteU();
  }
  ...
  function rewardCardU(r, g) {
    const owned = rewardOwnedU(g, r.id), active = rewardActiveU(g, r.id), currency = ['Gold','Diamond','XP'].includes(r.currency) ? r.currency : 'Gold';
    const symbol = currency === 'Gold' ? '🪙' : currency === 'Diamond' ? '💎' : '⚡';
    const action = owned ? `<button class="u-reward-action ${active ? 'active' : ''}" onclick="upgradeEquipReward('${escU(r.id)}')">${active ? 'Equipped · Unequip' : 'Equip / Use'}</button>` : `<button class="u-reward-action" onclick="upgradeBuyReward('${escU(r.id)}')">Buy · ${r.price} ${currency}</button>`;
    return `<article class="u-reward-card ${String(r.rarity || '').toLowerCase()}" data-reward-currency="${escU(currency)}" data-reward-search="${escU(`${r.name} ${r.category} ${r.description} ${r.rarity} ${currency}`.toLowerCase())}"><div class="u-reward-head"><span class="u-rarity">${escU(r.rarity || 'Rare')}</span><span class="u-reward-icon">${escU(r.icon || '✦')}</span></div><h3>${escU(r.name)}</h3><p>${escU(r.description)}</p><div class="u-reward-meta"><span>${escU(r.category)}</span><b>${owned ? (active ? 'ACTIVE' : 'OWNED') : `${symbol} ${r.price} ${currency}`}</b></div>${action}</article>`;
  }
  function renderRewardShopU() {
    const s = settingsU(), g = gameU(), info = levelInfoU(s.totalXpEarned), catalog = rewardCatalogU(), categories = [...new Set(catalog.map(r => r.category))], selected = selectedUpgradeCategoryU && categories.includes(selectedUpgradeCategoryU) ? selectedUpgradeCategoryU : '';
    selectedUpgradeCategoryU = selected;
    const categoryTabs = [['','All Rewards'], ...categories.map(c => [c,c])].map(([value,label]) => `<button class="u-category-tab ${selected === value ? 'active' : ''}" onclick="selectUpgradeRewardCategoryU('${escU(value)}')">${escU(label)}</button>`).join('');
    const visible = selected ? catalog.filter(r => r.category === selected) : catalog;
    renderShell(`<div class="u-premium-page"><header class="u-vip-header"><button class="u-back" onclick="navigate('dashboard')">←</button><div><span>GOLD · DIAMOND · XP REWARDS</span><h1>Reward Shop</h1><p>${visible.length} of ${catalog.length} rewards · category-wise browsing · 3 currencies</p></div></header><section class="u-wallet-grid"><div><small>🪙 Gold</small><b>${g.gold.toLocaleString()}</b><span>Gold balance</span></div><div><small>💎 Diamond</small><b>${g.diamonds.toLocaleString()}</b><span>Diamond balance</span></div><div><small>⚡ XP</small><b>${numU(s.xpBalance).toLocaleString()}</b><span>Spendable XP</span></div><div><small>🎒 Owned</small><b>${Object.keys(g.inventory).filter(k => g.inventory[k]?.ownedAt).length}</b><span>Rewards owned</span></div></section><section class="u-xp-progress"><div class="u-row-between"><strong>Level ${info.level} progress</strong><span>${info.percent}%</span></div><div class="u-progress"><i style="width:${info.percent}%"></i></div><small>Study questions, complete exams and review mistakes to earn XP.</small></section><div class="u-shop-tools"><input id="uRewardSearch" type="search" placeholder="Search rewards..." oninput="filterUpgradeRewardsU(this.value)"></div><nav class="u-category-tabs" aria-label="Reward categories">${categoryTabs}</nav><div class="u-reward-grid" id="uRewardGrid">${visible.map(r => rewardCardU(r, g)).join('')}</div></div>`, { topbar: false });
  }
  window.selectUpgradeRewardCategoryU = category => { selectedUpgradeCategoryU = String(category || ''); renderUpgradeRouteU(); };
  window.filterUpgradeRewardsU = value => { const q = String(value ?? document.getElementById('uRewardSearch')?.value ?? '').toLowerCase().trim(); document.querySelectorAll('#uRewardGrid [data-reward-search]').forEach(card => { card.hidden = !!(q && !card.dataset.rewardSearch.includes(q)); }); };
