(() => {
  ...
  async function exportQuestionBankData() {
    try {
      const subjects = await dbGetAll('subjects');
      const topics = await dbGetAll('topics');
      const questions = await dbGetAll('questions');
      const data = { type: 'admission-hub-question-bank-export', version: 1, exportedAt: Date.now(), counts: { subjects: subjects.length, topics: topics.length, questions: questions.length }, subjects, topics, questions };
      const url = URL.createObjectURL(new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }));
      const link = document.createElement('a');
      link.href = url;
      link.download = `admission-hub-question-bank-${todayKey()}.json`;
      document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url);
      toast(`${questions.length.toLocaleString()}টি question export হয়েছে`);
    } catch (_) { toast('Question Bank export করা যায়নি।'); }
  }
  window.exportQuestionBankData = exportQuestionBankData;
