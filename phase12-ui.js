(() => {
  ...
  window.renderDashboard = function phase12Dashboard() {
    ...
    const commandPages = [0,1,2].map(page => dashboardTools.slice(page*4,page*4+4).map(([icon,title,subtitle,route]) => `
      <button class="command-card" onclick="navigate('${route}')">
        <span class="command-icon" aria-hidden="true">${icon}</span>
        <span class="command-title">${title}</span>
        <span class="command-subtitle">${subtitle}</span>
...
phase12Style.textContent = `
  ...
  @media(max-width:430px){.command-slide{gap:10px}.command-slide .command-card{min-height:120px;padding:15px}.section-heading span{font-size:10px}}@media(prefers-reduced-motion:reduce){*,*:before,*:after{animation-duration:.01ms!important;transition-duration:.01ms!important}}
`;
