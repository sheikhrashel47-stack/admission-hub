(() => {
  ...
  const COURSE_DEFS = {
    ...
    'prottoy-master': { id: 'prottoy-master-exact-native-v1', path: './courses/prottoy-master/index.html', hash: 'e285153168db59a84db610408f9f641168ea7bfc76a3018baaf51ca70867e7f8', label: 'প্রত্যয় Master', title: 'প্রত্যয় • Visual Admission Master Guide — Interactive', subtitle: 'Bangla 2nd Paper · 200 MCQ · Interactive guide' }
  };
  ...
  const normalizeQuestions = questions => questions.map((q, index) => ({ ...q, d: normalizeDifficulty(q.d || q.lvl), id: qId(q, index), number: Number(q.number || q.n || index + 1), question: q.q || q.question || '', options: Array.isArray(q.o) ? q.o : (q.options || []), answer: Number(q.a ?? q.answer ?? 0), family: q.t || q.topic || q.cat || 'Source MCQ', difficulty: normalizeDifficulty(q.d || q.lvl), explanation: q.e || q.explanation || '' }));
  ...
  const library = () => {
    removeNativeState();
    const cards = Object.entries(COURSE_DEFS).map(([key, course]) => `<section class="source-course-card"><div class="source-course-icon">${esc(course.label)}</div><div><span>SOURCE-LOCKED · NATIVE COURSE</span><h2>${esc(course.title)}</h2><p>${esc(course.subtitle)}</p><small>Original source content preserved · Native MCQ · Separate Course progress</small></div><button class="btn" onclick="navigate('source-courses/${key}')">Open Course →</button></section>`).join('');
  ...
  const loadSource = () => {
    ...
    const request = fetch(`${def.path}?nativeCourse=v2`, { cache: 'force-cache' }).then(response => {
      ...
      const transformed = scripts
        ...
        .replace(/\n\}\);\s*$/, '\n}') + (key === 'prottoy-master' ? '\nwindow.__sourceCourseMCQ = Array.isArray(window.G?.mcqs) ? window.G.mcqs : []; window.__sourceCourseActiveKey = "prottoy-master";' : '');
