...
`;
  ...
  async function applyCategoryImages(category, images) { const records = categoryRecords(String(category || '').toUpperCase()); if (!records.length || !images.length) return toast('Import করার মতো category/image পাওয়া যায়নি।'); const total = Math.min(records.length, images.length); for (let index = 0; index < total; index += 1) { const record = records[index]; await dbPut(STORE, normalizeRecord({ ...record, id:record.id, order:record.order, imageDataUrl:images[index], createdAt:record.createdAt })); } await loadRecords(true); closeModal(); refreshCategoryResults(); toast(`${total}টি image serial অনুযায়ী card-এ বসানো হয়েছে।`); }
  async function importCategoryImages(category, input) { const files = [...(input?.files || [])]; if (!files.length) return; try { toast('Images serial অনুযায়ী প্রস্তুত হচ্ছে…'); await applyCategoryImages(category, await imageDataUrls(files)); } catch (_) { toast('Multiple image import করা যায়নি।'); } finally { if (input) input.value = ''; } }
  async function importCategoryPdf(category, input) { const file = input?.files?.[0]; if (!file) return; try { toast('PDF pages image হিসেবে পড়া হচ্ছে…'); await applyCategoryImages(category, await pdfPageImages(file)); } catch (_) { toast('PDF parse করা যায়নি। Internet connection থাকলে আবার চেষ্টা করো।'); } finally { if (input) input.value = ''; } }
  ...
  function openCategorySettings(category) {
    ...
    confirmModal(
      ...
      `${normalized} category-এর ${records.length.toLocaleString()}টি vocabulary স্থায়ীভাবে মুছে ফেলতে চান? এই কাজটি undo করা যাবে না।`,
  ...
  async function deleteCategoryVocabulary(category) {
    ...
    try {
      ...
      toast(`${normalized} category-এর ${records.length.toLocaleString()}টি vocabulary মুছে ফেলা হয়েছে।`);
    } catch (_) {
      toast('Category-এর vocabulary মুছে ফেলা যায়নি। আবার চেষ্টা করুন।');
    }
  }
