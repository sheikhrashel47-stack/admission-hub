...
`;
  ...
  const api = {
    ...
    async saveParsed() { const valid = state.parser.records.filter(record => record.valid); if (!valid.length) return toast('No valid vocabulary to save'); const seenNormalized = new Set(state.records.map(record => record.normalized)); let saved = 0, duplicates = 0; for (const source of valid) { const record = normalizeRecord(source); if (seenNormalized.has(record.normalized)) duplicates++; await dbPut(STORE, record); seenNormalized.add(record.normalized); saved++; } await loadRecords(true); state.parser = { text:'', records:[], stage:'input' }; toast(`${saved} vocabulary saved${duplicates ? ` · ${duplicates}টি ডুপ্লিকেট নতুন রেকর্ড হিসেবে রাখা হয়েছে` : ''}`); navigate(route('bank')); },
    ...
    openCategorySettings(key) { const cat = String(key || '').toUpperCase(); const target = cat === 'ALL' ? '' : cat; const records = target ? state.records.filter(row => categoryOf(row.word) === target) : state.records.slice(); const label = target || 'ALL'; if (!records.length) return toast(`${label} category-তে ডিলিট করার মতো vocabulary নেই।`); confirmModal('Category vocabulary ডিলিট', `${label} category-র <b>${records.length}টি</b> vocabulary চিরতরে মুছে যাবে। এটি আর ফেরত আনা যাবে না।`, async () => { for (const row of records) { try { await dbDel(STORE, row.id); } catch (_) {} } await loadRecords(true); refreshCategoryResults(); toast(`${records.length}টি vocabulary মুছে ফেলা হয়েছে।`); }, 'চিরতরে ডিলিট', true); },
