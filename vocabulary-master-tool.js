...
`;
  ...
  function renderBank() {
    ...
    const body = `<main class="vm-page">${heading('VOCABULARY BANK', 'Browse A–Z', state.records.length ? 'একটি অক্ষর নির্বাচন করে সেই category-এর শব্দ দেখুন।' : 'আপনার vocabulary এখনো যোগ করা হয়নি।')}<div class="vm-section-head"><h2>Categories</h2><span class="vm-count">${state.records.length.toLocaleString()} words</span></div>${state.records.length ? `<div class="vm-az-grid">${'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map(letter => `<button class="vm-letter" onclick="VocabularyMaster.openCategory('${letter}')"><b>${letter}</b><small>${counts[letter].toLocaleString()} words</small></button>`).join('')}</div>` : emptyBank()}<div style="margin-top:16px"><button class="btn secondary" onclick="navigate('${route('parser')}')">⚙️ Vocabulary Parser</button></div></main>`;
