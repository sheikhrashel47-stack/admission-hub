(() => {
  ...
  function topicRow(t) {
    ...
    return `<article class="q-nav-card" role="button" tabindex="0" onclick="openRedesignedTopic('${qEsc(t.id)}')" onkeydown="if(event.key==='Enter')openRedesignedTopic('${qEsc(t.id)}')">
      <span class="q-topic-icon" aria-hidden="true">📄</span><div class="q-nav-info"><strong>${qEsc(t.name)}</strong></div>
      <div class="q-topic-count">${qs.length} Questions</div><div class="q-manage-actions"><button type="button" class="q-manage-btn" onclick="event.stopPropagation();ahRenameTopic('${qEsc(t.id)}')">Rename</button><button type="button" class="q-manage-btn danger" onclick="event.stopPropagation();ahDeleteTopic('${qEsc(t.id)}')">Delete</button></div><span class="q-nav-arrow" aria-hidden="true">›</span>
    </article>`;
  }
  ...
  window.ahBulkDeleteQuestions = async ids => { const list=Array.isArray(ids)?ids:[]; if(!list.length||!confirm(`Delete ${list.length} selected questions?`))return; for(const id of list)await dbDel('questions',id); CACHE.questions=await dbGetAll('questions'); renderQuestionBankV2(); };
  ...
  let topicFeedObserver = null;
  function setupTopicFeedObserver() {
    if (topicFeedObserver) { topicFeedObserver.disconnect(); topicFeedObserver = null; }
    const sentinel = document.querySelector('.q-feed-body .explorer-load');
    if (!sentinel || !('IntersectionObserver' in window)) return;
    topicFeedObserver = new IntersectionObserver(entries => {
      if (entries.some(entry => entry.isIntersecting)) {
        topicFeedObserver.disconnect();
        window.loadMoreTopicQuestions();
      }
    }, { root: null, rootMargin: '700px 0px', threshold: 0 });
    topicFeedObserver.observe(sentinel);
  }
  ...
  function renderFeed() {
    ...
    requestAnimationFrame(setupTopicFeedObserver);
  }
