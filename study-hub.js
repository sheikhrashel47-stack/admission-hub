(() => {
  ...
  function dashboardCleanup() {
    ...
    if (!page) return;
    page.querySelectorAll('.command-carousel,.command-dots,.command-track,[data-special-study-tools],[data-study-tools-entry],.st-dashboard-entry,[data-p3-command],.p3-dashboard-v3,.p3-dashboard-shell').forEach((node) => node.remove());
    Array.from(page.querySelectorAll('section,article,.dashboard-section')).forEach((node) => {
      const text = (node.textContent || '').replace(/\s+/g, ' ');
      if (/^(Your Command Center|Special Study Tools|Study Tools)\b/i.test(text.trim()) || /Special Study Tools|Open Study Tools|7 utilities|ভুল খাতা/.test(text)) node.remove();
    });
    page.querySelectorAll('button[onclick]').forEach((button) => {
      const onclick = button.getAttribute('onclick') || '';
      if (/navigate\(['"](?:mistakes|bhul-khata|study-tools|memorizing-33)['"]\)/.test(onclick)) button.remove();
    });
    if (!page.querySelector('[data-study-hub-dashboard-card]')) {
      const target = page.querySelector('.dashboard-v2') || page;
